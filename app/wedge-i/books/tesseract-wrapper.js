// Wedge AI Eye OCR adapter (plain JS on purpose so Next/Turbopack does not need
// to type-resolve the wrapped OCR runtime during production builds).

export const PSM = Object.freeze({
  OSD_ONLY: "0",
  AUTO_OSD: "1",
  AUTO_ONLY: "2",
  AUTO: "3",
...
async function loadRuntime() {
  if (typeof window === "undefined") throw new Error("Wedge AI Eye OCR is browser-only.");
  ...
  runtimePromise = new Promise((resolve, reject) => {
    ...
    if (ready?.createWorker) return resolve(ready);

    const previous = document.querySelector('script[data-wedge-tesseract="7"]');
    const finish = () => {
...
function evidenceScore(text) {
  ...
  return Math.min(30, letters / 8) + Math.min(10, digits / 8) + Math.min(24, money * 6) +
    Math.min(10, dates * 5) + (documentWords ? 12 : 0) + (supplierWords ? 14 : 0);
}

function inferredConfidence(text) {
...
function shouldRescue(result) {
  const text = resultText(result);
  const confidence = resultConfidence(result);
  if (!text || confidence < 48 || text.length < 90) return true;
  return !/(?:RM\s*)?\d[\d,]*[.,]\d{2}\b/i.test(text) && /invoice|receipt|resit|invois/i.test(text);
}

function canvasBlob(canvas) {
  return new Promise((resolve, reject) => {
    canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error("OCR rescue image could not be prepared.")), "image/png", 1);
  });
}

async function adaptiveThresholdBlob(source, region) {
  ...
  try {
    ...
    // Upscale small photos, but downscale giant prepared canvases so the rescue pass
    // stays below roughly 50-80 MB on normal browsers.
    const targetWidth = Math.min(2600, Math.max(sw, Math.round(sw * 2.1)));
    ...
    for (let y = 0; y < height; y += 1) {
      ...
      for (let x = 0; x < width; x += 1) {
        ...
        const value = Math.round(pixels.data[rgba] * 0.2126 + pixels.data[rgba + 1] * 0.7152 + pixels.data[rgba + 2] * 0.0722);
    ...
    const radius = Math.max(10, Math.min(26, Math.round(Math.min(width, height) * 0.009)));
    for (let y = 0; y < height; y += 1) {
...
export async function createWorker(...args) {
  ...
  worker.recognize = async (image, options, output) => {
    const first = await originalRecognize(image, options, output);
    if (!shouldRescue(first) || typeof window === "undefined" || !(image instanceof Blob)) return first;

    try {
      const rescued = [first];
      const adaptive = await adaptiveThresholdBlob(image);
      rescued.push(await originalRecognize(adaptive, options, output));
      ...
      if (portraitDocument) {
        ...
        await originalSetParameters({ ...activeParameters, tessedit_pageseg_mode: PSM.SPARSE_TEXT, preserve_interword_spaces: "1" });
        ...
        const headerResult = await originalRecognize(header, options, output);
        const footerResult = await originalRecognize(footer, options, output);
        ...
        const merged = mergeOcrText([resultText(headerResult), resultText(strongest), ...rescued.map(resultText), resultText(footerResult)]);
        return {
          ...strongest,
          data: {
            ...(strongest?.data ?? {}),
            text: merged,
            confidence: Math.max(...rescued.map(resultConfidence), inferredConfidence(merged)),
          },
        };
      }
