...
export default function ProductAccessGate({
  ...
  useEffect(() => {
    ...
    async function openProduct() {
      ...
      try {
        const result = await productRequest<Session>(product, "/api/saas/auth/session");
        ...
        if (product === "books") {
          await bootstrapBooksCloudState();
        }
  ...
  if (loading) {
    return (
      <main className="grid min-h-screen place-items-center bg-[#f4f0e8] text-[#5c4a30]">
        Checking {config.name} access and cloud records…
      </main>
    );
  }
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[#f4f0e8] px-6 text-center text-[#20282c]">
        <div>
          ...
          <a className="mt-6 inline-block rounded-full bg-[#b7892e] px-6 py-3 font-bold text-white" href={`${config.basePath}/login`}>
            Product Login
          </a>
        </div>
      </main>
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[#f4f0e8] px-6 text-center text-[#20282c]">
        <section className="max-w-xl rounded-[2rem] border border-[#20282c]/10 bg-white p-8 shadow-xl">
          ...
          <p className="mt-5 leading-7 text-[#657074]">
            Your existing records remain stored. Please contact the Wedge team to continue using {config.name}.
          </p>
          <a href="/" className="mt-7 inline-flex rounded-full bg-[#20282c] px-7 py-4 font-bold text-white">
          ...
          <button
            ...
            className="mt-4 block w-full text-sm text-[#657074] underline"
          >
            Log out
          </button>
        </section>
