...
function OvertimeDecisionCard({ request, onDecision }: { request: OvertimeRequest; onDecision: (id: string, decision: "approved" | "rejected", allocation?: OvertimeDecisionPayload) => Promise<void> }) {
  ...
  return <article className="rounded-2xl border border-white/10 bg-black/10 p-5">
    ...
    {request.status === "approved" && <p className="mt-3 text-xs text-emerald-200/75">Payroll: {((request.payableMinutes || 0) / 60).toFixed(2)}h · Replacement hours: {((request.replacementCreditMinutes || request.replacementMinutes || 0) / 60).toFixed(2)}h · Ratio {Number(request.ratio || 0).toFixed(2)}</p>}
  </article>;
}
