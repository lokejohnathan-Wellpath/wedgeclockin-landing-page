...
type SiteDraft = { businessName: string; industry: string; description: string; whatsapp: string; phone: string; address: string; hours: string; facebook: string; instagram: string; accent: string; heroImage: string; offerings: Offering[] };
type PreviewPage = "home" | "offerings" | "contact";

const STORAGE_KEY = "wedgeweb_draft_v1";
const defaultDraft: SiteDraft = { businessName: "", industry: "Salon & Spa", description: "", whatsapp: "", phone: "", address: "", hours: "Monday – Saturday, 10:00 AM – 7:00 PM", facebook: "", instagram: "", accent: "#b58a72", heroImage: "", offerings: [{ name: "", price: "", description: "" }] };
...
export default function WedgeWebPage() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      if (saved) { try { setDraft({ ...defaultDraft, ...JSON.parse(saved) }); } catch { localStorage.removeItem(STORAGE_KEY); } }
      setLoaded(true);
    }, 0);
  ...
  function saveDraft() { localStorage.setItem(STORAGE_KEY, JSON.stringify(draft)); setMessage("Draft saved on this device."); }
  ...
  function uploadImage(event: ChangeEvent<HTMLInputElement>) { const file = event.target.files?.[0]; if (!file) return; if (file.size > 1_500_000) { setMessage("Use an image smaller than 1.5 MB for this preview."); return; } const reader = new FileReader(); reader.onload = () => update("heroImage", String(reader.result || "")); reader.readAsDataURL(file); }
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top_right,rgba(200,164,103,.13),transparent_32%),#090d10] text-[#f4efe6]">
    ...
    {mode === "build" ? <section className="mx-auto grid max-w-7xl gap-8 px-6 py-10 lg:grid-cols-[.9fr_1.1fr]">
      ...
      <form onSubmit={generate} className="space-y-5 rounded-[2rem] border border-white/10 bg-[#151b1f] p-6 sm:p-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm text-white/65">Brand colour<input type="color" value={draft.accent} onChange={(event) => update("accent", event.target.value)} className="mt-2 h-12 w-full rounded-xl border border-white/10 bg-[#0c1114] p-2"/></label><label className="block text-sm text-white/65">Hero photo<input type="file" accept="image/*" onChange={uploadImage} className="mt-2 block w-full rounded-xl border border-white/10 bg-[#0c1114] p-3 text-xs text-white/50"/></label></div>
...
function PreviewShell({ draft, page, setPage, words, whatsappUrl, chatOpen, setChatOpen, onEdit }: { draft: SiteDraft; page: PreviewPage; setPage: (page: PreviewPage) => void; words: { page: string; verb: string; fallback: string }; whatsappUrl: string; chatOpen: boolean; setChatOpen: (open: boolean) => void; onEdit: () => void }) {
  const accent = draft.accent || "#b58a72";
  return <section className="mx-auto max-w-7xl px-4 py-7 sm:px-6"><div className="mb-4 flex flex-col gap-3 rounded-2xl border border-white/10 bg-[#151b1f] p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.2em] text-[#c8a467]">Free preview</p><p className="mt-1 text-sm text-white/45">Payment connection comes next. Publishing is not enabled yet.</p></div><div className="flex gap-2"><button onClick={onEdit} className="rounded-full border border-white/15 px-4 py-2 text-sm">Edit</button><button onClick={() => alert("Publishing will unlock after WedgeWeb payment is connected.")} className="rounded-full bg-[#c8a467] px-4 py-2 text-sm font-bold text-[#111416]">Pay & Publish — Coming Next</button></div></div>
    <div className="relative min-h-[720px] overflow-hidden rounded-[2rem] bg-[#fffaf5] text-[#282321] shadow-2xl">
      ...
      {page === "home" && <div><div className="grid min-h-[490px] lg:grid-cols-2"><div className="flex flex-col justify-center px-8 py-14 sm:px-14"><p className="text-xs font-bold uppercase tracking-[.24em]" style={{ color: accent }}>{draft.industry}</p><h1 className="mt-5 text-4xl font-bold leading-tight sm:text-6xl">Welcome to {draft.businessName || "your new website"}.</h1><p className="mt-6 max-w-xl text-lg leading-8 text-black/60">{draft.description || words.fallback}</p><div className="mt-8 flex flex-wrap gap-3"><button onClick={() => setPage("offerings")} className="rounded-full px-6 py-3 font-bold text-white" style={{ backgroundColor: accent }}>Explore {words.page}</button><a href={whatsappUrl} target="_blank" className="rounded-full border border-black/15 px-6 py-3 font-semibold">WhatsApp Us</a></div></div><div className="min-h-[340px] bg-[#e8ddd3] bg-cover bg-center" style={draft.heroImage ? { backgroundImage: `url(${draft.heroImage})` } : { background: `linear-gradient(135deg,${accent}66,#eadfd5)` }} /></div><div className="grid gap-4 border-t border-black/8 px-8 py-8 sm:grid-cols-3 sm:px-14"><Info title="Opening hours" value={draft.hours}/><Info title="Location" value={draft.address || "Add your business location"}/><Info title="Direct enquiry" value="Chat with us through WhatsApp"/></div></div>}
      {page === "offerings" && <div className="px-8 py-12 sm:px-14"><p className="text-xs font-bold uppercase tracking-[.24em]" style={{ color: accent }}>WHAT WE OFFER</p><h1 className="mt-4 text-4xl font-bold">{words.page}</h1><div className="mt-9 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{draft.offerings.filter((item) => item.name).map((item,index) => <article key={index} className="rounded-3xl border border-black/8 bg-white p-6 shadow-sm"><div className="h-2 w-16 rounded-full" style={{ backgroundColor: accent }}/><h2 className="mt-6 text-xl font-bold">{item.name}</h2><p className="mt-2 font-semibold" style={{ color: accent }}>{item.price || "Contact us"}</p><p className="mt-4 text-sm leading-6 text-black/55">{item.description || "Ask us for details and availability."}</p><a href={whatsappUrl} target="_blank" className="mt-6 inline-block font-bold" style={{ color: accent }}>{words.verb} through WhatsApp →</a></article>)}</div></div>}
