  ...
  return (
    <main className="min-h-screen bg-[#101416] text-[#f4efe6]">
      <header className="mx-auto flex max-w-6xl items-center justify-between px-6 py-6">
        <div className="font-bold text-lg">🕒 WedgeCLOCKin</div>
      ...
      <section className="mx-auto grid max-w-6xl items-center gap-12 px-6 py-20 md:grid-cols-2">
        <div>
          ...
          <div className="mt-8 flex flex-wrap gap-3 text-sm text-white/60">
            <span className="rounded-full border border-white/10 px-4 py-2">Face Recognition</span>
            <span className="rounded-full border border-white/10 px-4 py-2">GPS Attendance</span>
            <span className="rounded-full border border-white/10 px-4 py-2">Leave Management</span>
            <span className="rounded-full border border-white/10 px-4 py-2">Payroll Export</span>
          </div>
          ...
          <div className="mt-12 flex flex-col gap-4 sm:flex-row">
            ...
            <button className="rounded-full border border-white/10 px-8 py-4 font-semibold text-white/80">
          ...
          <div className="mt-5 rounded-2xl bg-white/5 p-5">
            ...
            {["Employee 1", "Employee 2", "Employee 3", "Employee 4"].map((name, i) => (
              <div key={name} className="flex justify-between py-2 text-sm">
                <span>{name}</span>
                <span className="text-[#d4ad63]">{i === 2 ? "Leave" : `09:0${i}`}</span>
              </div>
            ))}
          </div>
        </div>
