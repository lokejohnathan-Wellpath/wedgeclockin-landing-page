    ...
    <main className="overflow-hidden bg-[#0d1316] text-[#f3efe7]">
      ...
      <section id="platform" className="bg-[#f3efe7] px-6 py-24 text-[#20282c]">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-16 grid gap-0 border-y border-[#20282c]/10 md:grid-cols-2 xl:grid-cols-4">
      ...
      <section id="industries">
        {stories.map((story) => <article id={story.eyebrow === "WEDGE-SMARTPOS" ? "smartpos" : undefined} key={story.eyebrow} className={story.dark ? "bg-[#102029] text-[#f3efe7]" : "bg-[#f3efe7] text-[#20282c]"}><div className="mx-auto grid min-h-[520px] max-w-[1500px] lg:grid-cols-2"><div className={`${story.imageFirst ? "lg:order-2" : ""} flex items-center px-8 py-16 sm:px-14 lg:px-20`}><div className="max-w-lg"><p className={`text-xs font-bold tracking-[.24em] ${story.dark ? "text-[#d2aa62]" : "text-[#b08745]"}`}>{story.eyebrow}</p><h2 className="mt-6 font-serif text-4xl leading-[1.08] sm:text-5xl">{story.title}</h2><div className="mt-7 h-0.5 w-12 bg-[#d2aa62]" /><p className={`mt-7 text-base leading-8 ${story.dark ? "text-white/60" : "text-[#59666a]"}`}>{story.text}</p><a href={story.href} className={`mt-9 inline-flex rounded-md px-6 py-3 font-bold ${story.dark ? "bg-[#d2aa62] text-[#0d1316]" : "bg-[#20282c] text-[#f3efe7]"}`}>{story.action}</a></div></div><div className={`${story.imageFirst ? "lg:order-1" : ""} min-h-[380px] overflow-hidden ${"secondImage" in story ? "grid grid-rows-2" : ""}`}><img src={story.image} alt={story.eyebrow.toLowerCase()} className="h-full min-h-0 w-full object-cover transition duration-700 hover:scale-[1.025]" />{"secondImage" in story && story.secondImage && <img src={story.secondImage} alt="salon and spa appointment environment" className="h-full min-h-0 w-full object-cover transition duration-700 hover:scale-[1.025]" />}</div></div></article>)}
      </section>
      ...
      <section className="relative bg-[#0d1316] px-6 py-24">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[.85fr_1.15fr]"><div><p className="text-xs font-bold tracking-[.28em] text-[#d2aa62]">EVERYDAY IMPACT</p><h2 className="mt-6 font-serif text-5xl leading-tight">One platform.<br />Endless possibilities.</h2><p className="mt-6 max-w-lg leading-8 text-white/60">Clock in securely, understand the business, build customer relationships and connect appointments with daily operations.</p><div className="mt-9 flex flex-wrap gap-3"><a href="/wedgeweb" className="rounded-md bg-[#d2aa62] px-7 py-4 font-bold text-[#0d1316]">Start Free</a><a href="/manager-login" className="rounded-md border border-[#5e8983]/60 px-7 py-4 font-semibold text-[#b9d7d2]">Open Manager Desk</a></div></div><div className="grid gap-4 sm:grid-cols-2"><div className="rounded-2xl border border-white/10 bg-white/[.035] p-7"><p className="text-sm text-white/40">WedgeCLOCKin</p><p className="mt-3 text-2xl font-bold text-[#f1dfbc]">People and attendance</p></div><div className="rounded-2xl border border-white/10 bg-white/[.035] p-7"><p className="text-sm text-white/40">Wedge-I</p><p className="mt-3 text-2xl font-bold text-[#f1dfbc]">Executive clarity</p></div><div className="rounded-2xl border border-white/10 bg-white/[.035] p-7"><p className="text-sm text-white/40">WedgeWeb</p><p className="mt-3 text-2xl font-bold text-[#f1dfbc]">Customer presence and CRM</p></div><div className="rounded-2xl border border-white/10 bg-white/[.035] p-7"><p className="text-sm text-white/40">Wedge-SmartPOS</p><p className="mt-3 text-2xl font-bold text-[#f1dfbc]">Clients and appointments</p></div></div></div>
      </section>
